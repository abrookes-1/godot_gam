...
onready var sprite = get_parent().get_parent().get_node("Melee").get_node("MeleeSprite")
...
func _ready():
	...
	get_parent().get_parent().get_node("Melee").get_node("MeleeSprite").connect("animation_finished", self, "_on_attack_complete")
